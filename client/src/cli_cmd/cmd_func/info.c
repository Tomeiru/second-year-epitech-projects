...
static void handle_info_transaction(client_t *client, void *data)
{
    UNUSED(client);
    UNUSED(data);
    puts("[INFO] Infos has successfully been called");
}

static bool info_handler(client_t *client, command_id_t cmd, list_t *transactions)
{
    ...
    handle_info_transaction, NULL, transactions)->id;
...
bool info_parser(client_t *client, int ac, char **av, list_t *transactions)
{
    ...
    for (int i = 0; i < 4; i++)
        if (client->use->state == (use_state_t)i)
            return (info_handler(client, INFO_CURRENT_USER_ID + i, transactions));
    return (ERROR_CMD);
}