...
struct my_ftrace_mmap_entry_vector {
    ...
    struct ftrace_mmap_entry *data;
};

// Guarantees that there will be at least self->length + length bytes available
// in the vector
static inline struct my_ftrace_mmap_entry_vector *
    my_ftrace_mmap_entry_vector_guarantee_can_expand(
    struct my_ftrace_mmap_entry_vector *self, size_t size)
{
    ...
    if (self->size + size >= current_allocated_size) {
        self->allocated_size = MY_MAX(self->size + size + 16,
            self->size * 3 / 2);
        self->data = (struct ftrace_mmap_entry *)my_xrealloc(self->data,
...
MY_ATTR_WARN_UNUSED_RESULT static inline struct my_ftrace_mmap_entry_vector *
    my_ftrace_mmap_entry_vector_new(void)
{
    struct my_ftrace_mmap_entry_vector *self =
        (struct my_ftrace_mmap_entry_vector *)my_xmalloc(sizeof(*self));

    self->size = 0;
    self->allocated_size = 0;
    self->data = NULL;
    return my_ftrace_mmap_entry_vector_guarantee_can_expand(self, 1);
}

// Inserts size bytes from data to self at position. data may not part of self
// (i.e. directly located within self->data)
static inline struct my_ftrace_mmap_entry_vector *
    my_ftrace_mmap_entry_vector_insert(struct my_ftrace_mmap_entry_vector *self,
    const struct ftrace_mmap_entry *data, size_t size, size_t position)
{
    if (self == NULL || data == NULL || size == 0 || (position > self->size))
        return self;
    my_ftrace_mmap_entry_vector_guarantee_can_expand(self, size);
...
static inline struct my_ftrace_mmap_entry_vector *
    my_ftrace_mmap_entry_vector_insert_single(
    struct my_ftrace_mmap_entry_vector *self,
    struct ftrace_mmap_entry item,
    size_t position)
{
