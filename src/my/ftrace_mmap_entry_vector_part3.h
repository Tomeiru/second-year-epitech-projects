...
#include "ftrace_mmap_entry_vector_part4.h"

// Append size items from data to self
static inline struct my_ftrace_mmap_entry_vector *
    my_ftrace_mmap_entry_vector_append(struct my_ftrace_mmap_entry_vector *self,
    const struct ftrace_mmap_entry *data, size_t size)
{
    return my_ftrace_mmap_entry_vector_insert(self, data, size, self->size);
}

// Same as append, but for a single item
static inline struct my_ftrace_mmap_entry_vector *
    my_ftrace_mmap_entry_vector_append_single(
    struct my_ftrace_mmap_entry_vector *self, struct ftrace_mmap_entry item)
{
    return my_ftrace_mmap_entry_vector_append(self, &item, 1);
}
...
static inline struct my_ftrace_mmap_entry_vector *
    my_ftrace_mmap_entry_vector_resize(struct my_ftrace_mmap_entry_vector *self,
    size_t size)
{
    if (self == NULL)
        return NULL;
    if (size >= self->allocated_size)
        my_ftrace_mmap_entry_vector_guarantee_can_expand(self,
            size - self->size);
    self->size = size;
    return self;
}
