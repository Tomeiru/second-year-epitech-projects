...
static const char *ssf_part2(ssf_opts_t *o, char *output_ptr,
    char (*output_string)[4096])
{
    bool didnt_find_nothing = false;

    for (size_t i = 0; i < o->enu->size && o->flags != 0; ++i)
        if (o->enu->data[i].value != 0 && o->enu->data[i].string && (o->flags &
            o->enu->data[i].value) == o->enu->data[i].value) {
            if (o->separator != '\0')
    ...
    if (o->flags != 0) {
        ...
        if (didnt_find_nothing || o->separator != '\0')
            output_ptr = STRACE_APPENDF_STRING(o->self, (*output_string),
                output_ptr, "%#jx", o->flags);
...
const char *strace_sprint_flags(ssf_opts_t *o)
{
    ...
    if (o->flags == 0 && o->enu->data->value == 0 && o->enu->data->string !=
        NULL) {
        if (o->separator != '\0')
            *output_ptr++ = o->separator;
