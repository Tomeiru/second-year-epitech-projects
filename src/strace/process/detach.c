...
static void wait_loop(struct strace *st, struct strace_process *self)
{
    unsigned signal;
    int wait_status;
    unsigned event;
    ...
    while (true) {
        if (waitpid(self->pid, &wait_status, 0) < 0) {
            if (errno == EINTR)
                continue;
            strace_print_error_message_errno(st, "waitpid(%u)", self->pid);
            break;
        }
        ...
        if (event == PTRACE_EVENT_STOP || signal == SIGTRAP)
            signal = 0;
        strace_do_ptrace_restart(st, PTRACE_DETACH, self, signal);
        break;
    }
}
