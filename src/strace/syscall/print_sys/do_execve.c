...
void strace_syscall_print_sys_do_execve(struct strace *self,
    struct strace_process *proc, unsigned int args_index)
{
    strace_syscall_print_path(self, proc, proc->syscall_arguments[args_index]);
    strace_syscall_print_next_argument(self);
    strace_syscall_print_argv(self, proc,
        proc->syscall_arguments[args_index + 1]);
    strace_syscall_print_next_argument(self);
    strace_syscall_print_argc(self, proc,
        proc->syscall_arguments[args_index + 2]);
}
