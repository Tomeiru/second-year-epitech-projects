...
static int sspqs_part2(sspqsp2_opts_t *o)
{
    ...
    if (o->buffer == NULL) {
        ftrace_print_error_message(o->self, "Memory exhausted while trying to "
            "allocate %zu bytes", o->allocation_size);
        ftrace_syscall_print_unavailable(o->self);
        return (-1);
    }
    r = ftrace_quote_string(&((sqs_opts_t){o->self, o->string, o->out_string,
        o->size, o->style}));
    ftrace_printf(o->self, "%s", o->out_string);
    free(o->buffer);
    return (r);
}
    ...
    struct ftrace *self, const char *string, size_t size, unsigned style)
{
    ...
    if (allocation_size / 4 != size) {
        ftrace_print_error_message(self, "requested %zu bytes exceeds %zu "
            "bytes limit", size, -(size_t)1u / 4);
        ftrace_syscall_print_unavailable(self);
        return (-1);
    }
    allocation_size += 1 + 2;
