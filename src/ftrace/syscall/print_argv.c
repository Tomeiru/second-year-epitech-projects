...
static bool do_after_copy_mem_fail(
    struct ftrace *self, __kernel_ulong_t address, size_t i)
{
    if (i == 0) {
        ftrace_syscall_print_address(self, address);
        return (false);
    }
    ftrace_syscall_print_array_next(self);
    ftrace_syscall_print_more_data_after_this(self);
    ftrace_syscall_print_address_comment(self, address);
    return (true);
}
...
{
    if (s->i == 0)
        ftrace_syscall_print_array_begin(s->self);
    if (s->c.ul == 0)
        return (false);
    if (s->i != 0)
        ftrace_syscall_print_array_next(s->self);
    if (s->i >= 32) {
        ftrace_syscall_print_more_data_after_this(s->self);
        return (false);
    }
    ftrace_syscall_print_string(s->self, s->proc, s->c.ul);
...
void sspap2_part2(sspa_state_t *s)
{
    ...
    while (true) {
        if (s->address < s->previous_address || ftrace_copy_mem_from_pid(
    ...
    ftrace_syscall_print_array_end(s->self);
}
