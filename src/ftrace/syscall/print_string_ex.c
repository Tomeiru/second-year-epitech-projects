...
static int sspse_part3(sspse_state_t *s)
{
    if (s->size > 32)
        ...
        s->string[s->size] = '\xff';
    s->printed_all = ftrace_quote_string(&((sqs_opts_t){s->self, s->string,
        s->output_string, s->size, s->style})) &&
        s->length != 0 && ((s->style & STRACE_SYSCALL_PRINT_QUOTE_0_TERM) ||
        s->length > 32);
    ftrace_printf(s->self, "%s", s->output_string);
    if (s->printed_all != 0)
...
static int sspse_part2(sspse_state_t *s)
{
    ...
    if (s->style & STRACE_SYSCALL_PRINT_QUOTE_0_TERM)
        s->r = ftrace_copy_str_from_pid(&((scsfp_opts_t){
            s->self, s->proc->pid, s->address, s->size, s->string }));
    else
        s->r = ftrace_copy_mem_from_pid(&((scmfp_opts_t){
            s->self, s->proc->pid, s->address, s->size, s->string }));
    if (s->r < 0) {
