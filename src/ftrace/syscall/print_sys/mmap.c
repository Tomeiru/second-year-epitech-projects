...
static void print_flags(struct ftrace *self, __kernel_ulong_t flags)
{
    unsigned huget_val;

    ftrace_syscall_print_flags(self, &FLAGS_ENUM, flags & MAP_TYPE, "MAP_???");
    flags &= ~MAP_TYPE;
    huget_val = flags & ((uintmax_t)MAP_HUGE_MASK << MAP_HUGE_SHIFT);
    flags &= ~((uintmax_t)MAP_HUGE_MASK << MAP_HUGE_SHIFT);
    if (flags) {
        ftrace_printf(self, "|");
        ftrace_syscall_print_flags(self, &FLAGS_ENUM, flags, NULL);
    }
    if (huget_val != 0)
        ftrace_printf(self, "|%u<<MAP_HUGE_SHIFT", huget_val >> MAP_HUGE_SHIFT);
}
