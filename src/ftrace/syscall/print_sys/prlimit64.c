...
static const struct ftrace_enum_data RLIMIT_ENUM_DATA[] = {
...
static const struct ftrace_enum RLIMIT_ENUM = {
    ...
    struct ftrace *self, struct ftrace_process *proc)
{
    if (!(proc->flags & STRACE_PROCESS_IN_SYSCALL)) {
        ...
        ftrace_syscall_print_next_argument(self);
        ftrace_syscall_print_rlimit(self, proc, proc->syscall_arguments[2]);
        ftrace_syscall_print_next_argument(self);
    } else
        ftrace_syscall_print_rlimit(self, proc, proc->syscall_arguments[3]);
    return (0);
}
