...
static int check_x86_stuff(struct ftrace *self, struct ftrace_process *proc)
{
...
static int ssgn_part2(struct ftrace *self, struct ftrace_process *proc)
{
    struct ftrace_syscall_entry_buffer *p;

    if (ftrace_syscall_number_is_valid(proc->syscall_number)) {
        proc->syscall_entry = &STRACE_SYSCALL_ENTRIES[proc->syscall_number];
        proc->qualifier_flags = 0;
    } else {
        p = ftrace_zalloc(self, sizeof(*p));
        p->proc = proc;
...
int ftrace_syscall_get_number(struct ftrace *self, struct ftrace_process *proc)
{
    int r;
