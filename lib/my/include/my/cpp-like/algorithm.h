...
#define MY_COUNT(first_param, last, value) (__extension__({ \
    size_t result = 0;                 \
    __auto_type first = (first_param); \
    for (; first != (last); ++first)   \
        if (*first == (value))         \
...
#define MY_MAKE_MIN_MAX_COMPARE_ONCE( \
    a, b, unique_identifier_a, unique_identifier_b, operation) (__extension__ \
    ({ \
...
#define MY_MAX_ELEMENT(first_param, last)  (__extension__({ \
    __auto_type first = (first_param); \
